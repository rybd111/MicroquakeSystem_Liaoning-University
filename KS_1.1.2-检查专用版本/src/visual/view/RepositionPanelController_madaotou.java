 ...
 *
 */
public class RepositionPanelController_madaotou {
...
//	private double[][] senserInformation=new double[numbername.size()][4];
	...
	void onSure(ActionEvent event) throws MWException, ParseException {
		...
		case "O sel ":
			...
			t = pHashMap.get('O') / (Parameters.FREQUENCY + 200);
			break;
		case "P nhy":
			a = Parameters.SENSORINFO1[Parameters.diskNameNum][1];
			t = pHashMap.get('P') / (Parameters.FREQUENCY + 200);
			break;
		case "Q wmz ":
			a = Parameters.SENSORINFO1[Parameters.diskNameNum][2];
			t = pHashMap.get('Q') / (Parameters.FREQUENCY + 200);
			break;
		case "Z tbz ":
		...
		for (int i = 0; i < numbername.size(); i++) {
			...
			if (numbername.get(i).equals("O sel ")) {
				senserInformation[i][0] = pHashMap.get('O') / (Parameters.FREQUENCY + 200);// 相对时间???
			...
			if (numbername.get(i).equals("P nhy")) {
				senserInformation[i][0] = pHashMap.get('P') / (Parameters.FREQUENCY + 200);
			...
			if (numbername.get(i).equals("Q wmz ")) {
				senserInformation[i][0] = pHashMap.get('Q') / (Parameters.FREQUENCY + 200);
			...
			if (numbername.get(i).equals("Z tbz ")) {
